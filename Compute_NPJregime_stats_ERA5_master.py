...
log_file_name = "output_files/npjphase.log"
...
if model == 'cmip':
    data_path = os.path.join(input_data_path, input_dataset)   ## input file name
    logging.info("Input data file: "+data_path)
    ds_cmip = xr.open_dataset(data_path)
    u_data = ds_cmip.ua.sel(plev=int(plev),lat=slice(latS,latN,4),lon=slice(lonW,lonE,4))
    times = ds_cmip.time
